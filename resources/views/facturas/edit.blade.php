...
@section('title', 'Editar Factura - Martínez Service')
        ...
        <form action="{{ route('facturas.update', $factura->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="equipo_id" class="form-control" id="equipoSelect">
                    ...
                    @foreach($equipos as $equipo)
                        <option value="{{ $equipo->id }}" data-cliente="{{ $equipo->cliente_id }}" {{ old('equipo_id', $factura->equipo_id) == $equipo->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="reparacion_id" class="form-control" id="reparacionSelect">
                    ...
                    @foreach($reparaciones as $reparacion)
                        <option value="{{ $reparacion->id }}" data-equipo="{{ $reparacion->equipo_id }}" data-total="{{ $reparacion->total ?? 0 }}" {{ old('reparacion_id', $factura->reparacion_id) == $reparacion->id ? 'selected' : '' }}>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div class="form-group">
                    ...
                    <input type="number" name="subtotal" id="subtotal" class="form-control" step="0.01" min="0" value="{{ old('subtotal', $factura->subtotal) }}" required>
                    @error('subtotal')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="impuestos" id="impuestos" class="form-control" step="0.01" min="0" value="{{ old('impuestos', $factura->impuestos ?? 0) }}">
                    @error('impuestos')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="total" id="total" class="form-control" step="0.01" min="0" value="{{ old('total', $factura->total) }}" required readonly style="background-color: #f8f9fa; font-weight: 600;">
                @error('total')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="fecha_emision" class="form-control" value="{{ old('fecha_emision', $factura->fecha_emision ? \Carbon\Carbon::parse($factura->fecha_emision)->format('Y-m-d') : date('Y-m-d')) }}" required>
                @error('fecha_emision')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
