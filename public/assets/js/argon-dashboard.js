...
function toggleSidenav() {
  if (!sidenav || !body) return;
  if (body.classList.contains(className)) {
    body.classList.remove(className);
    setTimeout(function () {
      if (sidenav) sidenav.classList.remove('bg-white');
    }, 100);
    if (sidenav) sidenav.classList.remove('bg-transparent');
  } else {
...
if (html) {
  html.addEventListener("click", function (e) {
    if (body && body.classList.contains('g-sidenav-pinned') && e.target && !e.target.classList.contains('sidenav-toggler-line')) {
      body.classList.remove(className);
    }
  });
...
function navbarColorOnResize() {
  ...
  // Re-verificar sidenav cada vez que se ejecuta la función por si se eliminó del DOM
  var currentSidenav = document.getElementById('sidenav-main');
  if (!currentSidenav || !currentSidenav.classList) {
    return;
  }
  
  if (window.innerWidth > 1200) {
    var currentReferenceButtons = document.querySelector('[data-class]');
    if (currentReferenceButtons && currentReferenceButtons.classList && currentReferenceButtons.classList.contains('active') && currentReferenceButtons.getAttribute('data-class') === 'bg-transparent') {
      currentSidenav.classList.remove('bg-white');
    } else {
      currentSidenav.classList.add('bg-white');
    }
  } else {
    currentSidenav.classList.add('bg-white');
    currentSidenav.classList.remove('bg-transparent');
  }
}

// Solo registrar el event listener si sidenav existe
